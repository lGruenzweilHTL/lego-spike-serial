...
class TestFirmwareFactory(unittest.TestCase):
    def test_add_command_no_params(self):
        ...
        content = "\n".join(lines)
        self.assertIn('if payload.lower() == "ping":', content)
        self.assertIn("print('pong')", content)
    ...
    def test_add_command_with_params(self):
        command = {
            ...
            "code": ["print(f'Moving at {speed} in {direction}')"]
        }
        lines = []
        firmware_factory.add_command(lines, command)
        
        content = "\n".join(lines)
        self.assertIn('if payload.lower().startswith("move "):', content)
    ...
    def test_from_file_integration(self):
        ...
        try:
            ...
            self.assertIn('if payload.lower() == "test":', fw_str)
            self.assertIn('result = 42', fw_str)
